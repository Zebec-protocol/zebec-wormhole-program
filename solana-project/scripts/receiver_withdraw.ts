...
let feeVaultATA = new anchor.web3.PublicKey(
    feeVaultATAA
);
...
const create_and_execute = async() => {
    ...
    let feeAccountTemp = await create_fee_account(feeReceiver, zebecProgram.programId);
    ...
    const tx = await program.rpc.createTransactionReceiverWithdraw(
        zebecProgram.programId,
        accounts,
        data,
