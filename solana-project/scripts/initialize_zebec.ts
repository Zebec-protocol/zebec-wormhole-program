...
const createUserAssociatedWalletAndMint = async (
	...
	if (mint) {
		...
		try {
			const txFundTokenSig = await provider.sendAndConfirm(txFundTokenAccount, [KEYPAIR]);
		} catch (error) {
			console.log(error);
		}
	}
...
const config_acc = async () => {
	const [config, bumps] = await PublicKey.findProgramAddress([Buffer.from("config")], program.programId);
...
const init_pda_ata = async () => {
	...
	depositorHash = tryNativeToUint8Array("0x30Fbf353f4f7C37952e22a9709e04b7541D5A77F", CHAIN_ID_BSC);
	receiverHash = tryNativeToUint8Array("0x30ca5c53ff960f16180aada7c38ab2572a597676", CHAIN_ID_BSC);
	...
	console.log("PDA Signer: " + pdaSignerTemp);

	let pdaSigner = pdaSignerTemp;

	const [pdaReciverTemp] = await anchor.web3.PublicKey.findProgramAddress([receiverHash, chainId], program.programId);
	...
	let [zebecVault] = await anchor.web3.PublicKey.findProgramAddress([pdaSigner.toBuffer()], zebecProgram.programId);
	...
	let withdrawDatatemp = await withdrawData("withdraw_token", pdaSigner, zebecProgram.programId, tokenMint);
	fs.writeFileSync("StaticAddress/withdrawData.txt", withdrawDatatemp.toBase58());
};

const init_fee_vault_ata = async () => {
	let fee_vault = new anchor.web3.PublicKey(fs.readFileSync("StaticAddress/feeVault.txt").toString());
...
const init_fee_account_zebec = async () => {
	...
	const [fee_vault, __] = await anchor.web3.PublicKey.findProgramAddress(
		[fee_receiver.publicKey.toBuffer(), anchor.utils.bytes.utf8.encode(OPERATE)],
		zebecProgram.programId,
	);

	const [vault_data, _] = await anchor.web3.PublicKey.findProgramAddress(
		[fee_receiver.publicKey.toBuffer(), anchor.utils.bytes.utf8.encode(OPERATEDATA), fee_vault.toBuffer()],
		zebecProgram.programId,
	);

	fs.writeFileSync("StaticAddress/feeReceiver.txt", fee_receiver.publicKey.toBase58());
...
const initZEBECFeeVault = async () => {
	...
	let vault_data = new anchor.web3.PublicKey(fs.readFileSync("StaticAddress/vaultData.txt").toString());
